...
class homeViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    let today = Calendar(identifier: .gregorian).date(byAdding: .day, value: 0, to: Date())
    ...
    @IBOutlet weak var toEureka: UIButton!
    ...
    func serch(){
           let day = stringFromDate(date: today!, format: "M月dd日")
    ...
    func tableView(_ tableView: UITableView,
        ...
        return stringFromDate(date: today!, format: "M月dd日")
    }
    
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let label = UILabel()
        label.font = UIFont(name: "Helvetica-Bold", size: 24)
        label.text = stringFromDate(date: today!, format: "M月dd日") + "（今日）"
        return label
    }
